...
class FluroXrayCTApp(QMainWindow):
    ...
    def capture_single_exposure(self):
        ...
        try:
            ...
            udsSaveframe = self.undistort_image(frame)
            imsave(f"single_exposure{timestamp}.tiff", udsSaveframe.astype(np.uint16))
    ...
    def update_feed(self, frame):
        ...
        try:
            ...
            #frame = self.undistort_image_complex(frame)
            frame = self.undistort_image(frame)
    ...
    def take_median_stack(self):
        ...
        if median_image is not None:
            ...
            median_image = self.undistort_image(median_image)
            timestamp = datetime.now().strftime("%d-%m-%Y %H-%M")  # Format: dd-mm-YYYY HH:ii
    ...
    def take_dark(self):
        ...
        try:
            ...
            self.dark_frame = self.undistort_image(self.dark_frame)
    ...
    def take_flat(self):
        ...
        try:
            ...
            self.flat_frame = self.undistort_image(self.flat_frame)
    ...
    def applyAllCalibrationsOneSpot(framem, disable = 0):
    ...
    def calibrate_camera(self, image_path, grid_size , square_size , output_file="calibration.json"):
        """
        Calibrate the camera using a grid pattern image.

        Parameters:
        - image_path: Path to the calibration image (16-bit grayscale).
        - grid_size: Tuple (rows, cols) representing the number of inner corners in the grid.
        - square_size: Size of each square in the grid (in real-world units).
        - output_file: File to save calibration data (JSON format).

        Returns:
        - success: True if calibration is successful, False otherwise.
        """
        try:
            ...
            ret, corners = cv2.findChessboardCorners(img_8bit, grid_size, None)
            if not ret:
                raise ValueError("Chessboard corners not found.")
            criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 1e-5)
            corners = cv2.cornerSubPix(img_8bit, corners, (11, 11), (-1, -1), criteria)

            # Draw and display the corners
            img_with_corners = cv2.drawChessboardCorners(img_8bit, grid_size, corners, ret)
            ...
            print(f"Calibration successful. Data saved to {output_file}.")
            return True
    ...
    def undistort_image_complex(self, image):
        """
        Undistort a 16-bit image using camera calibration data from `calibration.json`.

        Parameters:
        - image: Input image (16-bit numpy array).

        Returns:
        - undistorted_image: The undistorted 16-bit image.
        """
        try:
            ...
            if not hasattr(self, 'camera_matrix') or self.camera_matrix is None or self.dist_coeffs is None:
                print("Loading calibration data from calibration.json...")
                ...
                self.map1, self.map2 = cv2.initUndistortRectifyMap(
                    self.camera_matrix,
                    self.dist_coeffs,
                    None,  # Rectification transform (None for identity)
                    self.camera_matrix,  # New optimal camera matrix
                    (w, h),
                    cv2.CV_16SC2  # Output map type
                )
                print("Rectification maps calculated.")

            # Debug rectification maps
            if self.map1 is None or self.map2 is None:
                raise ValueError("Rectification maps are not initialized.")

            # Apply rectification maps to the input image
            undistorted_image = cv2.remap(image, self.map1, self.map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)

            print("Undistortion applied successfully.")
            return undistorted_image.astype(np.uint16)

        except FileNotFoundError:
            print("Calibration file calibration.json not found. Run interactive calibration first.")
            return image
    ...
    def interactive_calibration_fisheye(self, grid_size, square_size):
        calibration_folder = "calibrationimages"
        calibration_file = "calibration.json"
        ...
        print("Press SPACE to save a frame for calibration.")
        print("Press 'f' to finish and process the saved frames.")
        print("Press ESC to exit calibration.")

        try:
            while True:
                # Capture frame and convert to 8-bit
                frame = self.camera.capture()

                frame_8bit = cv2.normalize(frame, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
                if frame_shape is None:
                    frame_shape = frame.shape[::-1]  # Set frame shape based on the first valid frame
                    print(f"Set frame shape to: {frame_shape}")


                vis_image = frame_8bit.copy()
                cv2.putText(vis_image, "Press SPACE to capture frames", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

                cv2.imshow("Calibration Feed", vis_image)
                key = cv2.waitKey(250)

                if key == 32:  # SPACE key
                    # Detect checkerboard corners
                    ret, corners = cv2.findChessboardCorners(frame_8bit, grid_size)
                    if ret:
                        print(f"Checkerboard detected. Saving frame {frame_index}.")
                        corners = cv2.cornerSubPix(frame_8bit, corners, (11, 11), (-1, -1),
                                                   criteria=(cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001))
                        frame_path = os.path.join(calibration_folder, f"frame_{frame_index:04d}.png")
                        cv2.imwrite(frame_path, frame_8bit)
                        frame_index += 1
                        cv2.drawChessboardCorners(vis_image, grid_size, corners, ret)
                        
                    else:
                        print("Checkerboard not detected. Try adjusting the position.")
                        cv2.putText(vis_image, "Checkerboard not found!", (10, 60),
            ...
            if frame_shape is None:
                print("No valid frames were captured. Aborting.")
                return False

            # Prepare object points
            objp = np.zeros((grid_size[0] * grid_size[1], 1, 3), np.float32)
            objp[:, 0, :2] = np.mgrid[0:grid_size[1], 0:grid_size[0]].T.reshape(-1, 2) * square_size

            objpoints, imgpoints = [], []
            print("Processing saved frames...")

            for filename in sorted(os.listdir(calibration_folder)):
                frame_path = os.path.join(calibration_folder, filename)
                frame = cv2.imread(frame_path, cv2.IMREAD_GRAYSCALE)
                if frame is None:
                    print(f"Skipping invalid file: {filename}")
                    continue

                ret, corners = cv2.findChessboardCorners(frame, grid_size)
                if ret:
                    objpoints.append(objp)
                    corners = cv2.cornerSubPix(frame, corners, (11, 11), (-1, -1),
                                               criteria=(cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001))
                    imgpoints.append(corners)
                    print(f"Checkerboard detected in {filename}.")
                else:
                    print(f"Checkerboard not detected in {filename}. Skipping.")

            if len(objpoints) < 3:
                print("Not enough valid frames for calibration. Aborting.")
                return False
            ...
            try:
                ret, K, D, rvecs, tvecs = cv2.fisheye.calibrate(
                    objpoints, imgpoints, frame_shape, K, D, rvecs, tvecs, flags=flags,
            ...
            if not ret:
                print("Fisheye calibration unsuccessful.")
                return False
            ...
            print(f"Fisheye calibration completed successfully. Data saved to {calibration_file}.")
            return True
    ...
    def interactive_calibration(self, grid_size, square_size):
        calibration_folder = "calibrationimages"
        calibration_file = "calibration.json"
        os.makedirs(calibration_folder, exist_ok=True)
        frame_index = 0
        ...
        try:
            ...
            for filename in sorted(os.listdir(calibration_folder)):
                ...
                if ret:
                    ...
                    print(f"Chessboard not detected in {filename}. Skipping.")

            if len(objpoints) < 3:
                print("Not enough frames for calibration. Aborting.")
                return False

            # Perform standard calibration
            print("Performing standard calibration...")
            frame_shape = frame.shape[::-1]  # Width x Height
            ret, camera_matrix, dist_coeffs, _, _ = cv2.calibrateCamera(
                objpoints, imgpoints, frame_shape, None, None,
                flags=cv2.CALIB_RATIONAL_MODEL + cv2.CALIB_FIX_PRINCIPAL_POINT + cv2.CALIB_FIX_ASPECT_RATIO
            )
            ...
            print(f"Calibration completed successfully. Data saved to {calibration_file}.")
            return True
    ...
    def interactive_calibration_circles(self, grid_size, square_size):
        """
        Perform interactive calibration using a circular or asymmetric dot grid pattern.

        Parameters:
        - grid_size: List or tuple (rows, cols) of the inner points in the dot grid.
        - square_size: Size of the grid square (in real-world units, e.g., millimeters).
        """
        # Ensure grid_size is a tuple or list
        if not isinstance(grid_size, (list, tuple)) or len(grid_size) != 2:
            raise ValueError("grid_size must be a tuple or list of two integers (rows, cols).")
        ...
        # Prepare object points
        objp = np.zeros((grid_size[0] * grid_size[1], 3), np.float32)
        objp[:, :2] = np.mgrid[0:grid_size[1], 0:grid_size[0]].T.reshape(-1, 2) * square_size

        print("Starting interactive calibration...")
        print("Press SPACE to capture a frame. Press 'f' to finish calibration. Press ESC to cancel.")

        while True:
            # Capture and normalize the frame
            frame = self.camera.capture()  # Assuming the ZWO camera capture method
            frame_8bit = cv2.normalize(frame, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)

            # Display live feed
            vis_image = frame_8bit.copy()
            cv2.putText(vis_image, "Press SPACE to capture", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.imshow("Calibration Feed", vis_image)

            key = cv2.waitKey(150)
            if key == 32:  # SPACE key
                # Detect circles grid
                ret, centers = cv2.findCirclesGrid(frame_8bit, tuple(grid_size), flags=cv2.CALIB_CB_SYMMETRIC_GRID)
                if ret:
                    print(f"Pattern detected. Saving frame {frame_index}.")
                    objpoints.append(objp)
                    imgpoints.append(centers)
                    frame_path = os.path.join(calibration_folder, f"frame_{frame_index:04d}.png")
                    cv2.imwrite(frame_path, frame_8bit)
                    frame_index += 1
                    cv2.drawChessboardCorners(vis_image, tuple(grid_size), centers, ret)
                    cv2.imshow("Calibration Feed", vis_image)
                    cv2.waitKey(150)
                else:
                    print("Pattern not detected. Adjust the position.")
            elif key == ord('f'):  # Finish
                print("Finishing calibration...")
                break
            elif key == 27:  # ESC
        ...
        frame_shape = frame_8bit.shape[::-1]  # Width x Height
        ret, camera_matrix, dist_coeffs, _, _ = cv2.calibrateCamera(
            objpoints, imgpoints, frame_shape, None, None
        )
        ...
        print(f"Calibration completed. Data saved to {calibration_file}.")
        return True
    ...
    def calibrate_barrel_distortion(self):
        ...
        self.zoom_scale = self.load_setting("zoom_scale") or 1.0  # Default is 1.0 (no zoom)
        ...
        def update_parameters(*args):
            try:
                ...
                self.zoom_scale = 0.5 + (zoom_slider_value / 100.0)  # Map 0-150 to 0.5-2.0
    ...
    def performCalibration(self):
        ...
        #self.interactive_calibration_circles([30,20], 10);
